...
module.controller('ExchangePluginMyTradesController', function($scope, ngTableParams, $timeout, nxt) {
  $scope.tableParams = new ngTableParams({ page: 1, count: 100 }, {
    total: 0,
    getData: function($defer, params) {
      var trade, trades = $scope.myTrades.slice((params.page() - 1) * params.count(), params.page() * params.count());
      if (trades.length) {
  ...
  function reload() {
    $scope.tableParams.total($scope.myTrades.length);
